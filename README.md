...
2. **Transformation** – Maps VCOM data structures to Yuman format and stores relationships in a local database
3. **Loading** – Pushes processed information to Yuman and manages ticket lifecycle

All synchronization state is tracked in a local `vcom_yuman_mapping.db` database, with conflict detection and resolution built-in.
...
- **SupabaseAdapter** – Database operations and mapping storage
- **Sync Engine** – Orchestrates the three-phase synchronization process

### Data Flow

```
VCOM API → Local Snapshot → Diff Engine → Supabase DB → Yuman API
    ↑                                                       ↓
    ←──────────── Conflict Resolution ←─────────────────────
```
