...
class YumanAdapter:
    ...
    def _yuman_client_for_site(self, site_row) -> int:
        ...
        # ───────────────────────── YUMAN ➜ DB  (import snapshot) ───────────

    def import_clients_to_db(self, *, rows: list[dict] | None = None) -> None:
        if rows is None:
            rows = self.yc.list_clients()

        # ─── noms de colonnes autorisés (+ created_at obligatoire) ───────────
        client_field_names = {f.name for f in dc_fields(Client)} | {"created_at"}

        normalized: list[dict] = []
        for raw in rows:
            # 1) harmoniser la clé id → yuman_client_id
            if "yuman_client_id" not in raw and "id" in raw:
                raw = raw.copy()
                raw["yuman_client_id"] = raw.pop("id")

            if not raw.get("yuman_client_id"):
                logger.debug("⏭️  client sans id Yuman – ignoré: %s", raw.get("name"))
                continue

            # 2) filtrer les colonnes voulues
            filtered = {k: v for k, v in raw.items() if k in client_field_names}

            # 3) created_at obligatoire
            filtered["created_at"] = filtered.get("created_at") or _now_iso()

            # 4) instancier / sérialiser la dataclass (facultatif mais propre)
            client = Client(
                yuman_client_id = filtered["yuman_client_id"],
                code            = filtered.get("code"),
                name            = filtered["name"],
                address         = filtered.get("address"),
            )
            filtered.update(client.to_dict())   # s’assure du mapping complet
            normalized.append(filtered)

        if normalized:
            (self.sb.sb
                .table("clients_mapping")
                .upsert(normalized, on_conflict="yuman_client_id")
                .execute())

    # ------------------------------------------------------------------ #
    #  Yuman ➜ DB : import des équipements                               #
    # ------------------------------------------------------------------ #
    def _import_equipments_to_db(self, *, equips: Dict[str, Equipment] | None = None) -> None:
        """
        Alimente `equipments_mapping` avec les objets Equipment déjà
        normalisés par `fetch_equips()`.

        - Si `equips` est fourni ⇒ on l’utilise.
        - Sinon ⇒ on appelle `self.fetch_equips()` (une seule fois).

        Les lignes sans `site_id` (site non mappé) ou sans `yuman_material_id`
        sont ignorées pour respecter les contraintes NOT NULL / UNIQUE.
        """
        sb = self.sb.sb                                  # client PostgREST

        # ── snapshot préparé ──────────────────────────────────────────────
        if equips is None:
            equips = self.fetch_equips()                 # dict[vdev_id → Equipment]

        # ── index existant pour éviter les doublons ───────────────────────
        rows_db = (
            sb.table("equipments_mapping")
            .select("yuman_material_id,vcom_device_id,vcom_system_key")
            .execute()
            .data or []
        )
        ids_in_db  = {r["yuman_material_id"] for r in rows_db if r["yuman_material_id"]}
        keys_in_db = {(r["vcom_device_id"], r["vcom_system_key"]) for r in rows_db}

        # ── mapping vcom_system_key → site_id (Supabase) ──────────────────
        site_rows = (
        ...
        site_id_by_vkey = {r["vcom_system_key"]: r["id"] for r in site_rows if r["vcom_system_key"]}

        inserts, updates = [], []

        for eq in equips.values():                       # eq est un dataclass Equipment
            if not eq.yuman_material_id:                 # sécurité : hors snapshot Yuman
                continue

            site_id = site_id_by_vkey.get(eq.vcom_system_key)
            if not site_id:                              # site pas mappé → skip
                continue

            row = {
                "yuman_material_id": eq.yuman_material_id,
                "site_id":          site_id,             # NOT NULL garanti
                "category_id":      eq.category_id,
                "eq_type":          eq.eq_type,
                "name":             (eq.name or "").strip(),
                "brand":            (eq.brand or "").strip(),
                "model":            (eq.model or "").strip(),
                "serial_number":    (eq.serial_number or "").strip(),
                "vcom_device_id":   eq.vcom_device_id,
                "vcom_system_key":  eq.vcom_system_key,
                "created_at":       _now_iso(),          # valeur de secours obligatoire
            }

            key_tuple = (eq.vcom_device_id, eq.vcom_system_key)
            if eq.yuman_material_id in ids_in_db or key_tuple in keys_in_db:
                updates.append(row)
            else:
                inserts.append(row)
        ...
        if inserts:
            ...
            .upsert(
                inserts,
                on_conflict=["yuman_material_id", "vcom_device_id"],      # clé unique
                ignore_duplicates=True
            ).execute()

        for row in updates:                               # volume raisonnable
            sb.table("equipments_mapping") \
            .update(row) \
            .eq("yuman_material_id", row["yuman_material_id"]) \
            .execute()
